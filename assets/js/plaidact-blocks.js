(function (wp) {
	...
	wp.blocks.registerBlockType('plaidact/asso-directory', {
		title: __('Répertoire Asso complet', 'plaidact-breves-feed'),
		icon: 'index-card',
		category: 'widgets',
		attributes: {
			postsToShow: { type: 'number', default: 9 }
		...
		edit: function (props) {
			return el('div', {}, [
				el(InspectorControls, {},
					el(PanelBody, { title: __('Paramètres Répertoire', 'plaidact-breves-feed') },
						el(RangeControl, {
							label: __('Nombre max', 'plaidact-breves-feed'),
							value: props.attributes.postsToShow,
							onChange: function (n) { props.setAttributes({ postsToShow: n || 9 }); },
