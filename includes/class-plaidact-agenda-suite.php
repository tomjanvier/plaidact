		...
		foreach ( $parts as $index => $part ) {
			...
			foreach ( $terms_map as $item ) {
				...
				$segment = (string) preg_replace_callback(
					...
					static function ( array $matches ) use ( $item, &$placeholders, &$placeholder_counter ): string {
						$token = '%%PLAIDACT_HOVER_' . $placeholder_counter . '%%';
						++$placeholder_counter;
						$placeholders[ $token ] = self::render_hover_token( (string) $item['type'], (string) $item['id'], (string) $matches[1] );
						return $token;
					},
					$segment
				);
			...
			if ( ! empty( $placeholders ) ) {
				$parts[ $index ] = strtr( $segment, $placeholders );
			}
		}
